...
alpha.a <- 0.5 # reduced infectiousness for asymptomatic individuals
alpha.late <- 0.5 # reduced infectiousness for late infections
...
incubation_fxn <- function(n) return(rtnorm(n, mean=5.2, sd=1, lower=4.1, upper=6.4)) # log normal parameters for days of incubation
...
run_sim <- function(sparams){
  with(sparams,{
    ...
    write.csv(sim_pop, paste0("data/workers/sim_pop.", risk.multi, ".", nDelay, ".", testingFreq,".", r,".csv"))
    return(sum(sim_pop$InfWorkDays, na.rm = TRUE))
  })
}

tf_array <- c(1:5, 7, seq(10,30,5))
...
out_fi <- paste0("data/sim.", risk.multi, ".", nDelay,".csv")
